...
update() {
    ...
    cd /usr/local/
    if  [ $# == 0 ] ;then
        wget -N --no-check-certificate -O /usr/local/x-ui-linux-${arch}.tar.gz https://github.com/cfwarp/x-ui/raw/main/x-ui-linux-${arch}.tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://github.com/cfwarp/x-ui/raw/main/x-ui-linux-${arch}.tar.gz"
    ...
    wget --no-check-certificate -O /usr/bin/x-ui https://github.com/cfwarp/x-ui/raw/main/x-ui.sh
...
update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://github.com/cfwarp/x-ui/raw/main/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
...
————————————————
 ...
 "
    ...
    remoteV=`wget -qO- https://github.com/cfwarp/x-ui/raw/main/install.sh | sed  -n 2p | cut -d '"' -f 2`
    localV=${xuiygV}
